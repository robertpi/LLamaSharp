$(document).ready(function () {
    ...
    let robotMessage = null;
    connection.on("newMessage", function (messageView) {
        let isMine = messageView.fromUserName === viewModel.myProfile().userName();

        if (isMine) {
            var message = new ChatMessage(messageView.id, [{ token: messageView.content }], messageView.timestamp, messageView.fromUserName, isMine, messageView.avatar);
            viewModel.chatMessages.push(message);
            $(".messages-container").animate({scrollTop: $(".messages-container")[0].scrollHeight}, 1000);
        } else {
            var next = { token: messageView.content, scores: messageView.tokenScores };

            if (robotMessage == null) {
                robotMessage = new ChatMessage(messageView.id, [next], messageView.timestamp, messageView.fromUserName, isMine, messageView.avatar);
                viewModel.chatMessages.push(robotMessage);
            } else {
                robotMessage.contents.push(next);
                if (messageView.content.indexOf(".") !== -1 || messageView.content.indexOf("?") !== -1  || messageView.content.indexOf("!") !== -1) {
                    robotMessage = null;
                }
            }
            $(".messages-container").animate({scrollTop: $(".messages-container")[0].scrollHeight}, 1000);
    ...
    function AppViewModel() {
        ...
        self.sendToRoom = function (message) {
            fetch('/api/Messages', {
                ...
                body: JSON.stringify({room: "lobby", content: message})
            });
        }
        ...
        self.stopConversation = function (message) {
            fetch('/api/Messages/Control', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ room: "lobby", content: "Stop" })
            });
        }
    ...
    function ChatMessage(id, contents, timestamp, fromUserName, isMine, avatar) {
        let self = this;
        self.id = ko.observable(id);
        self.contents = ko.observableArray(contents);
